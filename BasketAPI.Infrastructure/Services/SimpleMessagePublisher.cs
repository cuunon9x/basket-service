...
public class SimpleMessagePublisher : IMessagePublisher
{
    private readonly IPublishEndpoint _publishEndpoint;
    private readonly ILogger<SimpleMessagePublisher> _logger;

    public SimpleMessagePublisher(
        IPublishEndpoint publishEndpoint,
        ILogger<SimpleMessagePublisher> logger)
    {
    ...
    public async Task PublishAsync<T>(T message, CancellationToken cancellationToken = default) where T : class
    {
        try
        {
            ...
            _logger.LogInformation("Successfully published message of type {MessageType}",
                typeof(T).Name);
        }
        catch (Exception ex)
        {
